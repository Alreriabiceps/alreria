...
export default function AllChats({ currentUser, socketRef }) {
  ...
  useEffect(() => {
    ...
    const handler = ({ message }) => {
      if (!selectedFriend || message.sender._id !== selectedFriend._id || (isMobileView && !showChatAreaMobile) ) {
          ...
          <div className={styles.friendList}>
            {filteredFriends.length === 0 ? (
              ...
              return (
                ...
                >
                  <div className={`${styles.friendItemAvatar} ${isSelected ? styles.friendItemAvatarSelected : styles.friendItemAvatarUnselected}`}>
                    {getInitials(friend)}
                  </div>
                  <div className={styles.friendInfo}>
                    ...
                    <div className={`${styles.friendStudentId} ${isSelected ? styles.friendStudentIdSelected : ''}`}>
                      {friend.studentId}
                    </div>
                  </div>
                  {unreadCounts[friend._id] > 0 && (
                    <span className={styles.unreadBadge}>{unreadCounts[friend._id]}</span>
                  )}
                </div>
              );
            })}
          ...
          {selectedFriend ? (
            <>
              ...
              <div className={styles.chatPanelHeader}>
                {isMobileView && (
                    <button onClick={handleCloseChatMobile} className={styles.backButtonMobile}>
                      &larr; {/* Left arrow */} 
                    </button>
                )}
                <div className={styles.chatPanelAvatar}>{getInitials(selectedFriend)}</div>
                <div>
            ...
            !isMobileView && (
                <div className={styles.chatAreaPlaceholder}>
                    ...
                    <p className={styles.chatAreaPlaceholderText}>Select a friend to start chatting.</p>
                </div>
            )
          )}
        </div>
...
function ChatPanel({ friend, currentUser, socketRef }) {
  ...
  useEffect(() => {
    ...
    const messageHandler = ({ message }) => {
      if (message.sender._id === friend._id || message.recipient._id === friend._id) {
        setMessages(prev => [...prev, message]);
        if (message.sender._id === friend._id && document.hasFocus()) {
          socket.emit('chat:read', { messageId: message._id, friendId: currentUser.id });
        }
      }
    };
    ...
    const readHandler = ({ messageId }) => {
        setMessages(prev => prev.map(m => m._id === messageId ? { ...m, readAt: new Date().toISOString() } : m));
    };
  ...
  const handleSend = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/messages` , {
        ...
        body: JSON.stringify({ recipientId: friend._id, text: messageData.text })
      });
      const savedMessage = await response.json();
      if (response.ok) {
        setMessages(prev => prev.map(m => m._id === tempId ? { ...savedMessage.data, isSending: false } : m));
      } else {
        setMessages(prev => prev.map(m => m._id === tempId ? { ...m, isSending: false, error: true, status: 'Failed' } : m));
      }
    } catch (error) {
      setMessages(prev => prev.map(m => m._id === tempId ? { ...m, isSending: false, error: true, status: 'Failed' } : m));
    }
  ...
  return (
    <>
      <div className={styles.messagesContainer}>
        {loading && <div className={styles.loadingMessages}>Loading messages...</div>}
        {messages.map((msg, index) => {
          ...
          return (
            <div key={msg._id || index} className={`${styles.messageBubble} ${isSender ? styles.messageSent : styles.messageReceived}`}>
              {msg.text}
      ...
      {isTyping && <div className={styles.typingIndicator}>{friend.firstName || friend.studentId} is typing...</div>}
      <form onSubmit={handleSend} className={styles.chatInputArea}>
        <button type="button" onClick={() => setShowEmoji(!showEmoji)} className={styles.emojiButton}>
          😀
        </button>
        {showEmoji && (
          <div className={styles.emojiPickerContainer}>
            {EMOJIS.map(emoji => (
              <span key={emoji} onClick={() => insertEmoji(emoji)} className={styles.emojiCell}>
                {emoji}
              </span>
            ))}
          </div>
