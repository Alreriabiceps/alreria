...
const RpsPhase = ({
  ...
  const [applyShake, setApplyShake] = useState(false); // State for shake effect
  const countdownRef = useRef(null);
  const resultTimeoutRef = useRef(null);
  const shakeTimeoutRef = useRef(null); // Ref for shake timeout
  ...
  useEffect(() => {
    ...
    if (rpsResult) {
      ...
      setApplyShake(true);
      // Clear previous shake timeout if any
      if (shakeTimeoutRef.current) {
        clearTimeout(shakeTimeoutRef.current);
      }
      // Remove shake class after animation duration (match CSS)
    ...
    return () => {
      ...
      // Also clear shake timeout on unmount
      if (shakeTimeoutRef.current) {
        clearTimeout(shakeTimeoutRef.current);
  ...
  // Determine winner/loser status for animation
  const isPlayerWinner = rpsResult && !rpsResult.isDraw && rpsResult.winner;
  const isOpponentWinner = rpsResult && !rpsResult.isDraw && !rpsResult.winner;

  const renderRpsResult = () => (
    <div className={`${styles.rpsResultModal} ${showResultModal ? styles.show : styles.hide} ${applyShake ? styles.shakeEffect : ''}`}>
      <div className={styles.rpsResultContent}>
        <h2 className={styles.resultTitle}>Result!</h2>
        <div className={styles.rpsPlayers}>
          {/* Player */}
          <div className={`${styles.rpsPlayer} ${isPlayerWinner ? styles.winnerGlow : ''} ${isPlayerWinner ? styles.clashWinner : isOpponentWinner ? styles.clashLoser : ''}`}>
            <div
              className={`${styles.rpsPlayerChoice}`}
              // Attack direction depends on player position (left vs right)
              style={isPlayerWinner ? { '--attack-translate-x': '20px' } : {}}
            >
          ...
          {/* Opponent */}
          <div className={`${styles.rpsPlayer} ${isOpponentWinner ? styles.winnerGlow : ''} ${isOpponentWinner ? styles.clashWinner : isPlayerWinner ? styles.clashLoser : ''}`}>
            <div
              className={`${styles.rpsPlayerChoice}`}
              // Attack direction depends on player position (left vs right)
              style={isOpponentWinner ? { '--attack-translate-x': '-20px' } : {}}
            >
