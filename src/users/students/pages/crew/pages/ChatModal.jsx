...
export default function ChatModal({ open, onClose, friend, currentUser, socketRef, fixedToBubble, modalOffset = 0, onMinimize }) {
  ...
  messages.forEach((msg, i) => {
    ...
    if (msgDate !== lastDate) {
      renderedMessages.push(
        <div key={'date-' + msgDate + '-' + i} style={{ textAlign: 'center', color: '#82DFFF', fontWeight: 600, margin: '18px 0 8px', fontSize: 13 }}>
          {msgDate}
        </div>
      );
      lastDate = msgDate;
    ...
    renderedMessages.push(
      <div key={msg._id || i} style={{
        ...
        <div style={{
          background: isMine ? '#82DFFF' : '#232c3a',
          color: isMine ? '#0D131A' : '#E0F2F7',
          borderRadius: 8,
          padding: '7px 13px',
          maxWidth: 220,
  ...
  return (
    <>
      ...
      >
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #333', fontWeight: 700, fontSize: 18, color: '#82DFFF', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span>Chat with {(friend.firstName || friend.lastName) ? `${friend.firstName || ''} ${friend.lastName || ''}`.trim() : friend.studentId}</span>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {onMinimize && (
              <button onClick={onMinimize} style={{ background: 'none', border: 'none', color: '#fff', fontSize: 22, cursor: 'pointer', marginRight: 4 }} title="Minimize">–</button>
            )}
            <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#fff', fontSize: 22, cursor: 'pointer' }}>&times;</button>
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: 16, background: '#10151c' }}>
        ...
        <form onSubmit={handleSend} style={{ display: 'flex', borderTop: '1px solid #333', padding: 10, background: '#18202b', alignItems: 'center' }}>
          <button type="button" onClick={() => setShowEmoji(v => !v)} style={{ background: 'none', border: 'none', fontSize: 22, marginRight: 6, cursor: 'pointer' }} title="Emoji">😊</button>
          {showEmoji && (
            <div style={{ position: 'absolute', bottom: 60, left: 18, background: '#232c3a', borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.18)', padding: 8, zIndex: 3000, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {EMOJIS.map(e => (
          ...
          <input
            ...
            style={{ flex: 1, border: 'none', borderRadius: 6, padding: 10, fontSize: 15, background: '#232c3a', color: '#E0F2F7', outline: 'none' }}
            autoFocus
          />
          <button type="submit" style={{ marginLeft: 8, background: '#82DFFF', color: '#0D131A', border: 'none', borderRadius: 6, padding: '0 18px', fontWeight: 700, fontSize: 15, cursor: 'pointer' }}>Send</button>
        </form>
      </div>
    </>
  );
