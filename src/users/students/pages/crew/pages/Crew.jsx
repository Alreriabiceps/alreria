...
import { 
    FaUserPlus, FaUsers, FaGlobeAmericas, FaCheckCircle, FaHourglassHalf, FaSearch, FaCommentDots
} from 'react-icons/fa';
...
const Crew = () => {
    ...
    const getFilteredFriends = () => {
        ...
        const processedAcceptedFriends = acceptedFriends.map(item => {
            // If item has requester and recipient, assume it's a friend request object
            // and we need to extract the other user.
            if (item.requester && item.recipient && item.requester._id && item.recipient._id) {
                const otherUser = item.requester._id === currentUser.id ? item.recipient : item.requester;
        ...
        if (activeTab === 'all') {
            ...
            const combined = [...processedAcceptedFriends, ...pendingReceived, ...pendingSent];
            ...
            for (const user of combined) {
                ...
                } else if (user && !user._id && user.requestId && !seenUserIds.has(user.requestId)) { 
            ...
            friendsToDisplay = pendingRequests.map(req => {
                if (req.requester._id === currentUser.id) { // Sent by current user
                    return { 
                        ...req.recipient, 
    ...
    const filteredAndSortedFriends = getFilteredFriends()
        ...
        .sort((a, b) => {
            const nameA = `${a.firstName || ''} ${a.lastName || ''}${a.username || ''}${a.studentId || ''}`.toLowerCase();
            const nameB = `${b.firstName || ''} ${b.lastName || ''}${b.username || ''}${b.studentId || ''}`.toLowerCase();
        ...
        <div className={styles.crewContainer}>
            ...
            <section className={styles.addFriendSection}>
                ...
                <form onSubmit={handleSendRequest} className={styles.addFriendForm}>
                    ...
                    />
                    <button type="submit" className={styles.addFriendButton}>Send Request</button>
                </form>
            </section>

            {/* Feedback Message */}
                {feedback && (
                <div className={`${styles.feedbackMessage} ${feedback.type === 'error' ? styles.error : styles.success}`}>
                        {feedback.message}
                    <button onClick={() => setFeedback(null)} className={styles.closeFeedbackButton}>&times;</button>
                    </div>
                )}
            ...
            <div className={styles.controlsBar}>
                <div className={styles.tabs}>
                    <button onClick={() => setActiveTab('all')} className={activeTab === 'all' ? styles.activeTab : ''}>All Connections</button>
                    <button onClick={() => setActiveTab('accepted')} className={activeTab === 'accepted' ? styles.activeTab : ''}>Crew Members ({acceptedFriends.length})</button>
                    <button onClick={() => setActiveTab('pending')} className={activeTab === 'pending' ? styles.activeTab : ''}>Pending Requests ({pendingRequests.length})</button>
                    </div>
            ...
            <section className={styles.friendsListSection}>
                {(activeTab === 'all' && loadingAccepted && loadingPending) || (activeTab === 'accepted' && loadingAccepted) || (activeTab === 'pending' && loadingPending) ? (
                    <p className={styles.loadingMessage}>Loading connections...</p>
                ) : filteredAndSortedFriends.length === 0 ? (
                    <p className={styles.noFriendsMessage}>
                        {activeTab === 'pending' ? 'No pending requests.' : 
                         activeTab === 'accepted' ? 'No crew members yet. Add some!' :
                         'No connections found.'}
                    </p>
                ) : (
                    <ul className={styles.friendsList}>
                        {filteredAndSortedFriends.map(friend => (
                            <li key={friend._id || friend.requestId} className={styles.friendItem}>
                                <div 
                                                    className={styles.avatar}
                                    style={{ backgroundColor: getAvatarColor(friend.studentId || friend.username) }}
                                >
                                    {getInitials(friend)}
                                </div>
                                <div className={styles.friendInfo}>
                                    <span className={styles.friendName}>
                                        ...
                                        {!friend.firstName && !friend.lastName && (friend.studentId || friend.username)}
                                                </span>
                                    <span className={styles.friendStudentId}>{friend.studentId || friend.username}</span>
                                </div>
                                <div className={styles.friendActions}>
                                    {friend.status === 'pending-received' ? (
                                        <button onClick={() => handleAcceptRequest(friend.requestId)} className={`${styles.actionButton} ${styles.acceptButton}`}>
                                            <FaCheckCircle /> Accept
                                        </button>
                                    ) : friend.status === 'pending-sent' ? (
                                        <button onClick={() => handleCancelRequest(friend.requestId)} className={`${styles.actionButton} ${styles.cancelButton}`}>
                                            <FaHourglassHalf /> Cancel
                                                    </button>
                                    ) : activeTab === 'accepted' || (activeTab === 'all' && !friend.status) ? ( // Ensure it's an accepted friend
                                        <>
                                                    <button
                                            ...
                                            <button onClick={() => handleRemoveFriend(friend._id)} className={`${styles.actionButton} ${styles.removeButton}`}>
                                                    Remove
                                                </button>
                                        </>
                                    ) : null}
