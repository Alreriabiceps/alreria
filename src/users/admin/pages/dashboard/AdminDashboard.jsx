    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const activityResponse = await fetch(
          `${import.meta.env.VITE_BACKEND_URL}/api/admin/dashboard/activity`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
            ...
            {statCards.map((stat, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className={`p-1 sm:p-2 rounded-full ${stat.bgColor} ${stat.color}`}
                  >
                    {stat.icon}
                  </div>
                </div>
            ...
            <div className="lg:col-span-1 space-y-3 sm:space-y-4">
              ...
              <div className="card bg-base-100 p-2 sm:p-4 rounded-lg">
                ...
                <div className="space-y-2 sm:space-y-3">
                  {/* No pending actions implemented yet */}
                  <div className="text-center text-xs sm:text-sm text-base-content/70">
                    No pending actions (or not implemented)
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-2 sm:space-y-3">
                  {recentActivity.length === 0 ? (
                    ...
                    recentActivity.map((activity, index) => (
                      <div
                        key={index}
                        className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-base-200 rounded-lg"
                      >
                        <div
                          className={`p-1 sm:p-2 rounded-full ${activity.iconBgColor} ${activity.iconColor}`}
                        >
                          {activity.icon}
                        </div>
            ...
            <div className="lg:col-span-3 space-y-3 sm:space-y-4">
              ...
              <div className="card bg-base-100 p-2 sm:p-4 rounded-lg">
                ...
                <div className="overflow-x-auto -mx-2 sm:mx-0">
                  ...
                  {/* Leaderboard not implemented or no data */}
                  <div className="text-center text-xs sm:text-sm text-base-content/70">
                    Leaderboard not implemented or no data
                  </div>
                </div>
