...
import { MdAdd, MdSave, MdClose, MdVisibility, MdVisibilityOff, MdContentCopy, MdPreview, MdWarning } from "react-icons/md";
...
const AddStudents = () => {
        ...
        if (!value.trim()) {
          ...
          switch (field) {
            ...
            case 'password':
              ...
              break;
            case 'studentId':
              if (existingStudentIds.includes(value.trim()) || duplicateIds.has(value.trim())) {
                newFieldErrors[errorKey] = 'Student ID already exists or is duplicated';
                isValid = false;
              }
              break;
          }
  ...
  const clearDraft = () => {
    if (window.confirm('Are you sure you want to clear all data? This will remove any saved draft.')) {
      localStorage.removeItem('addStudents_draft');
      setStudents([{ firstName: "", lastName: "", studentId: "", grade: "", section: "", track: "", password: "" }]);
      setFieldErrors({});
      setError("");
  ...
  const hasErrors = Object.keys(fieldErrors).length > 0 || duplicateIds.size > 0;
      ...
      <div className="max-w-6xl mx-auto">
        <div className="bg-base-200 rounded-lg shadow-lg p-3 sm:p-4 lg:p-6">
          {guideMode && (
            <details className="mb-4 sm:mb-6 bg-base-200 border border-primary rounded p-3 sm:p-4">
              ...
              <ol className="mt-2 text-xs sm:text-sm text-base-content list-decimal list-inside space-y-1">
                ...
                <li>Use the password generator button to create secure passwords automatically.</li>
                <li>The form auto-saves your progress as a draft every 2 seconds.</li>
                <li>Red indicators show validation errors in real-time.</li>
                <li>Use the Preview button to review all student data before submission.</li>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
            <div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-primary">Add Students</h1>
              <p className="text-xs sm:text-sm text-base-content/70 mt-1">Create new student accounts for the system</p>
            </div>
          ...
          {duplicateIds.size > 0 && (
            <div className="alert alert-warning mb-4 sm:mb-6 text-sm">
              ...
              <span>Duplicate Student IDs found: {Array.from(duplicateIds).join(', ')}</span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            <div className="space-y-4 sm:space-y-6">
              {students.map((student, index) => (
                <div key={index} className="card bg-base-100 p-3 sm:p-4 lg:p-6 border">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
                    <div className="form-control">
                      <label className="label">
                        <span className="label-text font-medium text-xs sm:text-sm">First Name</span>
                      </label>
                      <input
                        type="text"
                        className={`input input-bordered bg-base-100 text-sm ${getFieldError(index, 'firstName') ? 'input-error' : ''}`}
                      ...
                      {getFieldError(index, 'firstName') && (
                        <label className="label">
                          <span className="label-text-alt text-error text-xs">{getFieldError(index, 'firstName')}</span>
                        </label>
                      )}
                    </div>
                    ...
                    <div className="form-control">
                      <label className="label">
                        <span className="label-text font-medium text-xs sm:text-sm">Last Name</span>
                      </label>
                      <input
                        type="text"
                        className={`input input-bordered bg-base-100 text-sm ${getFieldError(index, 'lastName') ? 'input-error' : ''}`}
                      ...
                      {getFieldError(index, 'lastName') && (
                        <label className="label">
                          <span className="label-text-alt text-error text-xs">{getFieldError(index, 'lastName')}</span>
                        </label>
                      )}
                    </div>
                    ...
                    <div className="form-control">
                      <label className="label">
                        <span className="label-text font-medium text-xs sm:text-sm">Student ID</span>
                      </label>
                      <div className="input-group">
                        <input
                          type="text"
                          className={`input input-bordered bg-base-100 text-sm font-mono flex-1 ${getFieldError(index, 'studentId') || duplicateIds.has(student.studentId) ? 'input-error' : ''}`}
                      ...
                      {(getFieldError(index, 'studentId') || duplicateIds.has(student.studentId)) && (
                        <label className="label">
                          <span className="label-text-alt text-error text-xs">
                            {getFieldError(index, 'studentId') || 'Duplicate Student ID'}
                          </span>
                        </label>
                      )}
                    </div>
                    ...
                    <div className="form-control">
                      <label className="label">
                        <span className="label-text font-medium text-xs sm:text-sm">Grade</span>
                      </label>
                      <select
                        className={`select select-bordered bg-base-100 text-sm ${getFieldError(index, 'grade') ? 'select-error' : ''}`}
                        value={student.grade}
                      ...
                      {getFieldError(index, 'grade') && (
                        <label className="label">
                          <span className="label-text-alt text-error text-xs">{getFieldError(index, 'grade')}</span>
                        </label>
                      )}
                    </div>
                    ...
                    <div className="form-control">
                      <label className="label">
                        <span className="label-text font-medium text-xs sm:text-sm">Section</span>
                      </label>
                      <input
                        type="text"
                        className={`input input-bordered bg-base-100 text-sm ${getFieldError(index, 'section') ? 'input-error' : ''}`}
                      ...
                      {getFieldError(index, 'section') && (
                        <label className="label">
                          <span className="label-text-alt text-error text-xs">{getFieldError(index, 'section')}</span>
                        </label>
                      )}
                    </div>
                    ...
                    <div className="form-control">
                      <label className="label">
                        <span className="label-text font-medium text-xs sm:text-sm">Track</span>
                      </label>
                      <select
                        className={`select select-bordered bg-base-100 text-sm ${getFieldError(index, 'track') ? 'select-error' : ''}`}
                        value={student.track}
                      ...
                      {getFieldError(index, 'track') && (
                        <label className="label">
                          <span className="label-text-alt text-error text-xs">{getFieldError(index, 'track')}</span>
                        </label>
                      )}
                    </div>
                    ...
                    <div className="form-control sm:col-span-2 xl:col-span-3">
                      <label className="label">
                        <span className="label-text font-medium text-xs sm:text-sm">Password</span>
                      </label>
                      <div className="input-group">
                        <input
                          type={showPasswords[index] ? "text" : "password"}
                          className={`input input-bordered bg-base-100 text-sm flex-1 ${getFieldError(index, 'password') ? 'input-error' : ''}`}
                        ...
                        >
                          {showPasswords[index] ? <MdVisibilityOff className="w-4 h-4" /> : <MdVisibility className="w-4 h-4" />}
                        </button>
                        <button
                          type="button"
                          className="btn btn-ghost btn-sm"
                      ...
                      {getFieldError(index, 'password') && (
                        <label className="label">
                          <span className="label-text-alt text-error text-xs">{getFieldError(index, 'password')}</span>
                        </label>
                      )}
                    </div>
                  </div>
                ...
                >
                  ...
                  {isSubmitting ? (
                    <>
                      ...
                      <span className="hidden sm:inline">Adding Students...</span>
                      ...
                      <tbody>
                        {students.map((student, index) => (
                          <tr key={index}>
                            ...
                            <td className="font-mono text-xs">{student.password}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
