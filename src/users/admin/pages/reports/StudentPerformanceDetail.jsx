...
const StudentPerformanceDetail = () => {
  ...
  useEffect(() => {
    const fetchStudentPerformance = async () => {
      ...
      setError('');
      try {
        const response = await fetch(`${backendUrl}/api/admin/students/${studentId}/performance`, {
          headers: {
        ...
        <div className="bg-base-200 rounded-lg shadow-lg p-3 sm:p-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm sm:text-base text-base-content/70 mt-1">
                  {studentData?.track} • {studentData?.section} • {studentData?.yearLevel}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <div className="card bg-base-100 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-base-content/70">Tests Completed</p>
                  <p className="text-2xl font-bold">
                    {performanceData?.overview.testsCompleted}/{performanceData?.overview.totalTestsAvailable}
                  </p>
                  <p className="text-xs text-info">
                    {Math.round((performanceData?.overview.testsCompleted / performanceData?.overview.totalTestsAvailable) * 100)}% completion
                  </p>
                </div>
              </div>
            </div>
            <div className="card bg-base-100 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-base-content/70">Class Rank</p>
                  <p className="text-2xl font-bold">
            ...
            <div className="card bg-base-100 p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-base-content/70">Current Streak</p>
                  <p className="text-2xl font-bold">{performanceData?.overview.currentStreak}</p>
          ...
          <div className="tabs tabs-boxed mb-4 sm:mb-6 bg-base-100 grid grid-cols-2 sm:grid-cols-4 gap-1 sm:gap-0">
            ...
            <button
              className={`tab tab-sm sm:tab-lg ${activeTab === 'history' ? 'tab-active' : ''}`}
            ...
            <button
              className={`tab tab-sm sm:tab-lg ${activeTab === 'skills' ? 'tab-active' : ''}`}
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Score Trend */}
                <div className="card bg-base-100 p-4">
                  <h3 className="text-lg font-semibold mb-4">Score Trend (Last 10 Tests)</h3>
                  <div className="h-64">
                    <Line
                      data={{
                ...
                <div className="card bg-base-100 p-4">
                  ...
                  <div className="h-64">
                    <Bar
                      data={{
                        labels: performanceData?.weeklyActivity.map(item => 
                          new Date(item.date).toLocaleDateString('en-US', { weekday: 'short' })
                        ) || [],
                        datasets: [
                          {
                            label: 'Tests Completed',
                            data: performanceData?.weeklyActivity.map(item => item.testsCompleted) || [],
                            backgroundColor: 'rgba(16, 185, 129, 0.8)',
                            yAxisID: 'y',
                          },
                          {
                            label: 'Time Spent (min)',
                            data: performanceData?.weeklyActivity.map(item => item.timeSpent) || [],
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="card bg-base-100 p-4">
                  <h3 className="text-lg font-semibold mb-4 text-success">Strengths</h3>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 bg-success/10 rounded-lg">
                      <span className="font-medium">Best Subject</span>
                      <span className="badge badge-success">{performanceData?.overview.bestSubject}</span>
                    </div>
                ...
                <div className="card bg-base-100 p-4">
                  <h3 className="text-lg font-semibold mb-4 text-warning">Areas for Improvement</h3>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 bg-warning/10 rounded-lg">
                      <span className="font-medium">Focus Subject</span>
                      <span className="badge badge-warning">{performanceData?.overview.improvementNeeded}</span>
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="card bg-base-100 p-4">
                  ...
                  <div className="h-64">
                    <Bar
                      data={{
                        labels: performanceData?.subjectPerformance.labels || [],
                        datasets: [
                          {
                            label: 'Average Score (%)',
                            data: performanceData?.subjectPerformance.data || [],
                            backgroundColor: [
                              'rgba(59, 130, 246, 0.8)',
                              'rgba(16, 185, 129, 0.8)',
                              'rgba(245, 158, 11, 0.8)',
                ...
                <div className="card bg-base-100 p-4">
                  ...
                  <div className="space-y-3">
                    {performanceData?.subjectPerformance.labels.map((subject, index) => (
                      <div key={subject} className="flex items-center justify-between p-3 bg-base-200 rounded-lg">
                        <div>
                          <span className="font-medium">{subject}</span>
                          <p className="text-sm text-base-content/70">
                            {Math.floor(Math.random() * 5) + 3} tests completed
                          </p>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-sm text-base-content/70">avg score</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <tbody>
                    {performanceData?.testHistory.map((test) => (
                      <tr key={test.id}>
                        ...
                        <td>
                          <span className="badge badge-outline">{test.subject}</span>
                        </td>
                        <td>
                          <span className={`font-bold ${test.score >= 80 ? 'text-success' : test.score >= 60 ? 'text-warning' : 'text-error'}`}>
                            {test.score}%
                          </span>
                        </td>
          ...
          {activeTab === 'skills' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="card bg-base-100 p-4">
                  <h3 className="text-lg font-semibold mb-4">Bloom's Taxonomy Performance</h3>
                  <div className="h-64">
                    <Radar
                      data={{
                ...
                <div className="card bg-base-100 p-4">
                  <h3 className="text-lg font-semibold mb-4">Cognitive Skills Analysis</h3>
                  <div className="space-y-3">
                    {performanceData?.bloomsPerformance.labels.map((skill, index) => (
                      <div key={skill} className="flex items-center justify-between p-3 bg-base-200 rounded-lg">
                        <span className="font-medium">{skill}</span>
                        <div className="flex items-center gap-2">
                          <div className="w-24 bg-base-300 rounded-full h-2">
                            <div
                              className="bg-primary h-2 rounded-full"
                              style={{ width: `${performanceData?.bloomsPerformance.data[index]}%` }}
                            ></div>
                          </div>
