...
import { MdEdit, MdDelete, MdSave, MdClose, MdCheck, MdClose as MdCloseIcon, MdAnalytics, MdViewList, MdSearch, MdFilterList, MdFileDownload, MdCheckBox, MdCheckBoxOutlineBlank, MdSelectAll, MdClear, MdInfo, MdCheckCircle, MdError, MdWarning, MdAutorenew, MdCloud, MdCloudDone, MdBookmark, MdQuiz, MdSchedule, MdBarChart, MdCalendarToday, MdPlayArrow, MdPause, MdVisibility } from "react-icons/md";
import { FaChartPie, FaHistory, FaCalendarAlt, FaClock, FaChartLine } from 'react-icons/fa';
...
const CurrentSchedules = () => {
  ...
  const saveToLocalStorage = useCallback((data) => {
    try {
      localStorage.setItem('schedules_state', JSON.stringify({
        ...data,
        timestamp: Date.now()
      }));
  ...
  const stats = useMemo(() => {
    ...
    const totalQuestions = schedules.reduce((sum, s) => sum + (s.questionIds?.length || 0), 0);
    const avgQuestionsPerSchedule = totalSchedules > 0 ? Math.round(totalQuestions / totalSchedules) : 0;
    
    // Subject distribution
    const subjectStats = subjects.map(subject => ({
      name: subject.subject,
      count: schedules.filter(s => s.subjectId?._id === subject._id).length,
      active: schedules.filter(s => s.subjectId?._id === subject._id && s.isActive).length,
      _id: subject._id
    }));
    ...
    for (let week = 1; week <= 52; week++) {
      ...
      if (weekSchedules.length > 0) {
        weeklyStats.push({
          ...
          questions: weekSchedules.reduce((sum, s) => sum + (s.questionIds?.length || 0), 0)
        });
      }
    }
    ...
    return {
      ...
      activationRate: totalSchedules > 0 ? Math.round((activeSchedules / totalSchedules) * 100) : 0,
      totalQuestions,
  ...
  const filteredAndSortedSchedules = useMemo(() => {
    ...
    const weekMatch = !selectedWeek || schedule.weekNumber === parseInt(selectedWeek);
      ...
      const searchMatch = !searchTerm || 
        schedule.subjectId?.subject?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    return filtered;
  }, [schedules, selectedWeek, selectedSubject, statusFilter, searchTerm, sortBy, sortOrder]);
  ...
  const handleBulkToggleActive = async (makeActive = true) => {
    ...
    if (successCount > 0) {
      ...
      fetchData();
      setSuccess(`Successfully ${makeActive ? 'activated' : 'deactivated'} ${successCount} schedules`);
    }
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Are you sure you want to delete ${selectedSchedules.length} schedules? This action cannot be undone.`)) {
      return;
    }
    ...
    // Update local state
    setSchedules(prev => prev.filter(s => !selectedSchedules.includes(s._id)));
    setSelectedSchedules([]);
    
    if (successCount > 0) {
      setSuccess(`Successfully deleted ${successCount} schedules`);
      saveToLocalStorage({ schedules: schedules.filter(s => !selectedSchedules.includes(s._id)) });
    }
    if (errorCount > 0) {
  ...
  return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-center">
          ...
          <p className="mt-2 text-base-content/70">Loading schedule dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 py-4 max-w-7xl">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-primary">Schedule Management</h1>
          <p className="text-base-content/70">Manage weekly schedules and monitor system activity</p>
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={() => window.location.href = '/admin/weeks/create'}
          >
       ...
       {guideMode && (
         <div className="alert alert-info mb-6">
           ...
           <div>
             ...
             <p className="text-sm">Use the dashboard to monitor schedule performance, manage schedules with bulk operations, and analyze weekly distribution patterns.</p>
           </div>
         </div>
       )}
         ...
         <div className="space-y-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
             ...
             <div className="stat bg-base-200 rounded-lg p-4">
               ...
               <div className="stat-value text-accent">{stats.totalQuestions}</div>
               <div className="stat-desc">Avg {stats.avgQuestionsPerSchedule} per schedule</div>
             </div>
             <div className="stat bg-base-200 rounded-lg p-4">
               ...
               <div className="stat-value text-info">{stats.recentlyCreated}</div>
                     ...
                     <div className="card-body p-4">
                       ...
                       <div className="w-full bg-base-300 rounded-full h-2 mt-2">
                         <div 
                           ...
                           style={{ width: `${subject.count > 0 ? (subject.active / subject.count) * 100 : 0}%` }}
                         ></div>
                       </div>
                     </div>
                   </div>
         ...
         <div className="space-y-6">
               ...
               {filteredAndSortedSchedules.map((schedule) => (
                 <div 
                   ...
                   className={`card bg-base-200 shadow hover:shadow-lg transition-all cursor-pointer ${
                     selectedSchedules.includes(schedule._id) ? 'ring-2 ring-primary' : ''
                   }`}
                 >
                   <div className="card-body p-4">
                     <div className="flex items-start justify-between mb-3">
                       ...
                       <div className="dropdown dropdown-end">
                         ...
                         <ul tabIndex={0} className="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52">
                           <li>
                             <button onClick={() => handleToggleActive(schedule._id)}>
              ...
              <table className="table w-full">
                         ...
                         <th>
                           <label>
                             <input 
                               type="checkbox" 
                               className="checkbox"
                               checked={selectedSchedules.length === filteredAndSortedSchedules.length && filteredAndSortedSchedules.length > 0}
                ...
                <tbody>
                      ...
                      <tr key={schedule._id}>
                           ...
                           <td>
                             <div className="font-medium">Week {schedule.weekNumber}</div>
                           </td>
                           <td>
           ...
           {filteredAndSortedSchedules.length === 0 && (
             <div className="text-center py-12">
               ...
               <h3 className="text-lg font-medium text-base-content/70 mb-2">No schedules found</h3>
               <p className="text-base-content/50 mb-4">
                 {searchTerm || selectedWeek || selectedSubject || statusFilter !== 'all' 
        ...
        <div className="modal-box max-w-4xl">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-base-content/70">Modify schedule details and question assignments</p>
            </div>
            <button 
              className="btn btn-ghost btn-sm"
            ...
            <div className="form-control">
              ...
              {editQuestions.length > 0 && (
                <div className="alert alert-info mb-3">
                  ...
                  <span className="text-sm">{editQuestions.length} questions selected for this weekly test</span>
                </div>
              )}
              ...
              <div className="bg-base-100 rounded-lg p-4 max-h-96 overflow-y-auto border">
                ...
                {currentQuestions.map((question) => (
                  ...
                  <div className="text-center py-8 text-base-content/50">
                    {editSubject ? 'No questions available for this subject' : 'Please select a subject first'}
                  </div>
                )}
              </div>
              {totalPages > 1 && (
